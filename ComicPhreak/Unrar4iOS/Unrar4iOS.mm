...
-(BOOL) _unrarOpenFile:(NSString*)rarFile mode:(NSInteger)mode{
    ...
    // Bugfix: Since comment buffer size was not initialized, it had a value in it which was
    // causing RAR to try and copy the comment into the buffer. By setting the size to zero,
    // the comment processing code is skipped. We do not use the comment for RAR anyway.
    flags->CmtBufSize = 0;
...
-(NSData *) extractStream:(NSString *)aFile {
	...
	// Bugfix: Deallocate the created bytes since NSData will retain it, but
	// first create the new NSData object
	NSData *uncompressedData = [NSData dataWithBytes:buffer length:length];
	delete buffer;
	buffer = Nil;
...
-(void) dealloc {
//MVDBG:ARC	[filename release];
//MVDBG:ARC	[super dealloc];
}
